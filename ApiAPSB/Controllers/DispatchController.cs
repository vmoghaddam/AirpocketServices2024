    ...
    public class DispatchController : ApiController
    {
        ...
        public IHttpActionResult PostDRPICSIGNNew(dto_sign dto)
        {
            try
            {
                ...
                {
                    dr.JLDatePICApproved = dt;
                    dr.SgnCPTLicNo = person.NDTNumber;
                    dr.PICId = employee != null ? employee.Id : -1;
        ...
        public IHttpActionResult ValidateOFPs(string fids)
        {
            ...
            try
            {
                ...
                var flights2 = _context.AppLegs.Where(q => _fids.Contains(q.ID) && q.FlightStatusID != 9).ToList();
                var flight_ids = flights2.Select(q => q.ID).ToList();
               

                var ofp_roots = _context.view_ofpb_root_report.Where(q => flight_ids.Contains(q.FlightID.Value)).ToList();
                ...
                var flights = _context.FlightInformations.Where(q => flight_ids.Contains(q.ID) && q.FlightStatusID != 9).ToList();
                //var ofpImports = _context.OFPImports.Where(q => _fids.Contains(q.FlightId)).Select(q => new
                //{
                //    q.FlightId,
                //    q.FlightNo,
