    ...
    public class JourneyLogController : ApiController
    {
        ...
        public HttpResponseMessage GetJourneyLogV4XLS(int fid)
        {
            ...
            try
            {
                ...
                Spire.License.LicenseProvider.SetLicenseKey(LData);
                Workbook workbook = new Workbook();
                var mappedPathSource = System.Web.Hosting.HostingEnvironment.MapPath("~/upload/" + "jlv4" + ".xlsx");
                workbook.LoadFromFile(mappedPathSource);
                Worksheet sheet = workbook.Worksheets[0];
                ...
                foreach (var x in legs)
                {
                    ...
                    //sheet.Range[ln_leg_assignedcockpit, 6].Text = format_to_time(x.PF2Time); 
                ...
                if (result.pos1) { sheet.Range[30, 3].Text = "*"; }
                if (result.pos2) { sheet.Range[31, 3].Text = "*"; }
