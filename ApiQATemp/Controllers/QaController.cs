...
namespace ApiQA.Controllers
{
    ...
    public class QaController : ApiController
    {
        ...
        [HttpGet]
        [Route("api/receiver/log/{entityid}")]
        public async Task<DataResponse> ReceiverLog(int entityid)
        {
            try
            {
                var record = context.QAFollowingUps.Single(q => q.Id == entityid);
                record.ReceiverDateVisit = DateTime.Now;
                context.SaveChanges();
                return new DataResponse()
                {
                    Data = null,
        ...
        [HttpGet]
        [Route("api/get/datevisit/{employeeId}")]

        public async Task<DataResponse> GetDateVisit(int employeeId)
        {

            try
            {
                var query = from x in context.QAFollowingUps
                            where x.DateVisit == null && x.ReferredId == employeeId
        ...
        [HttpGet]
        [Route("api/set/datevisit/{employeeId}/{type}")]
        public async Task<DataResponse> QASetDateVisit(int employeeId, int type)
        {
            try
            {
                switch (type)
                {
                    case 0:
                        var records0 = context.QAFollowingUps.Where(q => q.Type == 0 && q.ReferredId == employeeId && q.DateVisit == null).ToList();
                        foreach (var record0 in records0)
                        {
                            record0.DateVisit = DateTime.Now;
                        }
                        context.SaveChanges();
                        break;
                    case 1:
                        var records1 = context.QAFollowingUps.Where(q => q.Type == 0 && q.ReferredId == employeeId && q.DateVisit == null).ToList();
                        foreach (var record1 in records1)
                        {
                            record1.DateVisit = DateTime.Now;
                        }
                        context.SaveChanges();
                        break;
                    case 2:
                        var records2 = context.QAFollowingUps.Where(q => q.Type == 0 && q.ReferredId == employeeId && q.DateVisit == null).ToList();
                        foreach (var record2 in records2)
                        {
                            record2.DateVisit = DateTime.Now;
                        }
                        context.SaveChanges();
                        break;
                    case 3:
                        var records3 = context.QAFollowingUps.Where(q => q.Type == 0 && q.ReferredId == employeeId && q.DateVisit == null).ToList();
                        foreach (var record3 in records3)
                        {
                            record3.DateVisit = DateTime.Now;
                        }
                        context.SaveChanges();
                        break;
                    case 4:
                        var records4 = context.QAFollowingUps.Where(q => q.Type == 0 && q.ReferredId == employeeId && q.DateVisit == null).ToList();
                        foreach (var record4 in records4)
                        {
                            record4.DateVisit = DateTime.Now;
                        }
                        context.SaveChanges();
                        break;
                    case 5:
                        var records5 = context.QAFollowingUps.Where(q => q.Type == 0 && q.ReferredId == employeeId && q.DateVisit == null).ToList();
                        foreach (var record5 in records5)
                        {
                            record5.DateVisit = DateTime.Now;
                        }
                        context.SaveChanges();
                        break;
                    case 6:
                        var records6 = context.QAFollowingUps.Where(q => q.Type == 0 && q.ReferredId == employeeId && q.DateVisit == null).ToList();
                        foreach (var record6 in records6)
                        {
                            record6.DateVisit = DateTime.Now;
                        }
                        context.SaveChanges();
                        break;
                    case 7:
                        var records7 = context.QAFollowingUps.Where(q => q.Type == 0 && q.ReferredId == employeeId && q.DateVisit == null).ToList();
                        foreach (var record7 in records7)
                        {
                            record7.DateVisit = DateTime.Now;
                        }
                        context.SaveChanges();
                        break;
                    case 8:
                        var records8 = context.QAFollowingUps.Where(q => q.Type == 0 && q.ReferredId == employeeId && q.DateVisit == null).ToList();
                        foreach (var record8 in records8)
                        {
                            record8.DateVisit = DateTime.Now;
                        }
                        context.SaveChanges();
                        break;
                }
        ...
        public async Task<DataResponse> GetQAEmployee(int type, int entityId, int referrerId)
        {
            try
            {
                ...
                //    Name = q.Name,
                //    ReceiverEmployeeId = q.ReceiverEmployeeId,
                //    JobGroup = q.JobGroup,
                //    Mobile = q.Mobile,
                //    Referred = !ableToRefer.Contains(q.ReceiverEmployeeId) ? 0 : 1
                //}).ToList();
        ...
        [HttpGet]
        [Route("api/get/voyage/{id}")]
        public async Task<DataResponse> GetVoyageReport(int id)
        {
            try
            {
                var result = context.ViewQaVoyageReports.SingleOrDefault(q => q.Id == id);
        ...
        public async Task<DataResponse> QAReferr(List<QADto> dto)
        {
            try
            {
                ...
                foreach (var x in referInfo)
                {

                    DateTime? DeadLine = null;
                    if (x.DeadLine.Year != 1970)
                        DeadLine = x.DeadLine;


                    var cmd = "select case when isnull(send.id,-1) =-1 and  isnull(recive.id,-1)<>-1 then 3       when isnull(send.id,-1)<isnull(recive.id,-1) then 3      else 2  end as ReviewRes  from  (select top 1 a.* from QAFollowingUp a where EntityId=" + x.EntityId + " and type= " + x.Type + " and ReferrerId= " + x.ReferrerId + " and ReferredId = " + x.ReferredId + " order by id DESC) as send FULL JOIN (select top 1 a.* from QAFollowingUp a  where EntityId=" + x.EntityId + " and type=" + x.Type + " and ReferrerId=" + x.ReferredId + " order by id DESC) as recive on 1=1 full join ( select top 1 a.Id from QAFollowingUp a order by a.Id) as r on 1 =1";
                    var referred = context.Database.SqlQuery<int>(cmd).FirstOrDefault();


                    //var referred = context.Database.ExecuteSqlCommand("select cast(isnull(case when isnull(send.id,-1) =-1 and  isnull(recive.id,-1)<>-1 then 3  when isnull(send.id,-1)>isnull(recive.id,-1) then 3 else 2 end,-1) as int) as ReviewRes  from  (select top 1 a.* from QAFollowingUp a where EntityId= 109 and type= 0 and ReferrerId=4539 order by id) as send FULL JOIN (select top 1 a.* from QAFollowingUp a  where EntityId=109 and type= 0 and ReferrerId= 3960 order by id) as recive on 1=1");
                ...
                if (dto.isResponsible == true)
                {
                    ...
                    switch ((int)dto.Type)
                    {
                        ...
                        case 7:
                            ...
                            break;
                        case 8:
                            context.Database.ExecuteSqlCommand("update EFBASR set Status = 1,DateStatus = '" + test + "', StatusEmployeeId= " + dto.EmployeeId + ",Result = '" + dto.Result + "' where Id=" + dto.Id);
                            break;
                        case 9:
                            context.Database.ExecuteSqlCommand("update EFBVoyageReport set Status = 1,DateStatus = '" + test + "', StatusEmployeeId= " + dto.EmployeeId + ",Result = '" + dto.Result + "' where Id=" + dto.Id);
                            break;
                    }
                }
