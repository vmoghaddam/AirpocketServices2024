...
namespace ApiXLS.Controllers
{
    ...
    public class SafetyFormsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> get_regulations(int id)
        {

            try
            {
                var context = new Models.dbEntities();
                var result = context.ViewBookApplicableEmployees.Where(q => q.Category == "Regulations" && q.IsExposed == 1 && q.EmployeeId == id).OrderByDescending(q => q.No).ToList();
